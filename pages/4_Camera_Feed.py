...
if not get_device_base_url():
    st.info("No device URL set in the sidebar — connect your ESP32-CAM (or similar) and set its address to see the live feed here.", icon="ℹ️")
...
if image_bytes:
    ...
    st.warning("Couldn't fetch a snapshot from the device. Showing a placeholder — check the device is powered on and reachable at the URL in the sidebar.", icon="⚠️")
    st.image("https://placehold.co/800x450?text=No+Camera+Feed", use_container_width=True)
if auto:
