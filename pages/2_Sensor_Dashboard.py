...
if len(st.session_state["sensor_history"]) > 1:
    ...
    for col, color in colors.items():
        fig.add_trace(go.Scatter(x=df["timestamp"], y=df[col], mode="lines+markers", name=col.replace("_", " ").title(), line=dict(color=color, width=2), marker=dict(size=6, line=dict(width=1, color="#0a0812"))))
    fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color="#b8b7c9", family="Inter"), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0), margin=dict(l=10,r=10,t=10,b=10), xaxis=dict(showgrid=False,color="#7d7c94"), yaxis=dict(showgrid=True,gridcolor="rgba(255,255,255,0.06)",color="#7d7c94"), height=360)
