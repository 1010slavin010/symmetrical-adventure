...
def record_push_data(payload: dict) -> dict:
    ...
    _last_push_data = {"soil_moisture": round(float(soil), 1), "light_lux": round(float(light), 0),
        "temperature": round(float(temp), 1), "humidity": round(float(hum), 1),
        "timestamp": time.strftime("%H:%M:%S"), "source": "ESP32 (192.168.31.57)"}
    _last_push_time = time.time()
    return _last_push_data


def _demo_sensor_reading() -> dict:
    return {"soil_moisture": round(random.uniform(42, 60), 1), "light_lux": round(random.uniform(38000, 56000), 0),
        "temperature": round(random.uniform(23, 28), 1), "humidity": round(random.uniform(55, 75), 1),
        "timestamp": time.strftime("%H:%M:%S"), "source": "demo (waiting for ESP32)"}
    ...
    if base_url:
        try:
            ...
            if resp.status_code == 200:
                return record_push_data(resp.json())
        except Exception:
            pass
