...
st.set_page_config(page_title="AgroSentry — Farm Ops", page_icon="🌾", layout="wide")
...
def feature_card(col, icon, title, desc, target, chip_label, chip_class):
    with col:
        with st.container(border=True):
            ...
            st.markdown(f'<span class="chip {chip_class}">{chip_label}</span>', unsafe_allow_html=True)
...
def home():
    ...
    st.markdown("Edge-AI monitoring for early crop disease detection, environmental sensing, and climate-resilience alerts — all in one place.")
...
st.session_state["_pages"] = {"home": home_page, "ai": "pages/1_AI_Assistant.py", "sensor": "pages/2_Sensor_Dashboard.py", "alerts": "pages/3_Flood_Drought_Alerts.py", "camera": "pages/4_Camera_Feed.py", "disease": "pages/5_Disease_Detection.py"}

pg = st.navigation([home_page, ai_page, sensor_page, alerts_page, camera_page, disease_page], position="hidden")
pg.run()
